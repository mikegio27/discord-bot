...
class InvalidVoiceChannel(VoiceConnectionError):
    logger.error('Failed to get valid voice channel')
...
class MusicSource(discord.PCMVolumeTransformer):
    ...
    def __getitem__(self, item: str):
        """Access attributes similar to a dict.
        DELETE ME IF YOU CHOOSE TO DOWNLOAD
        """
    ...
    async def regather_stream(cls, data, *, loop):
        """Get stream again if you didnt download
        """
        loop = loop or asyncio.get_event_loop()
        loop = loop or asyncio.get_event_loop()
    ...
    async def player_loop(self):
        ...
        while not self.bot.is_closed():
            ...
            if not isinstance(source, MusicSource):
                try:
                    source = await MusicSource.regather_stream(source, loop=self.bot.loop)
...
class Music(commands.Cog):
    ...
    async def __error(self, ctx, error):
        if isinstance(error, commands.NoPrivateMessage):
            try:
                return await ctx.send('This command cannot be sent in a private message')
            except discord.HTTPException:
                pass
        elif isinstance(error, InvalidVoiceChannel):
            await ctx.send('Error connecting to voice channel. Please make sure you are in a valid channel or give me one.')

        logger.info('Ignoring exception in command {}:'.format(ctx.command))
    ...
    async def play_(self, ctx, *, search: str):
        ...
        source = await MusicSource.create_source(ctx, search, loop=self.bot.loop, download=False)
    ...
    async def queue_info(self, ctx):
        ...
        fmt = f"\n__Now Playing__:\n[{vc.source.title}]({vc.source.web_url}) | ` Duration {duration} Requested by: {vc.source.requester}`\n\n__Up Next:__\n" + fmt + f"\n**{len(upcoming)} songs in queue**"
